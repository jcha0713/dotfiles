...
class FakePi {
	private readonly commands = new Map<string, { handler: (args: string, ctx: any) => Promise<void> | void }>();
	private readonly listeners = new Map<string, Array<(event: any, ctx: any) => Promise<void> | void>>();
...
function findLastMessageByType(messages: SentMessage[], customType: string): SentMessage | undefined {
...
describe("milestone 5.2 integration", () => {
	...
	it("busy/queue semantics unchanged for review flow", async () => {
		await withTempProject(async (cwd) => {
			...
			const requestCount = pi.sentMessages.filter((entry) => entry.message?.customType === "sasu-review-request").length;
			...
			const store = await createMemoryStore(cwd);
			let completed = await store.queryEvents({ projectRoot: cwd, kinds: ["agent.review.completed"], limit: 10 });
			...
			completed = await store.queryEvents({ projectRoot: cwd, kinds: ["agent.review.completed"], limit: 10 });
	...
	it("/sasu-suggest plus suggestion actions write feedback", async () => {
		await withTempProject(async (cwd) => {
			...
			await pi.emit(
				"agent_end",
				{ messages: [{ role: "assistant", content: "1. OPEN src/app.ts — Start from application entrypoint" }] },
				ctx,
			);

			const store = await createMemoryStore(cwd);
			const generated = await store.queryEvents({ projectRoot: cwd, kinds: ["agent.suggestion.generated"], limit: 10 });
			const actions = await store.queryEvents({ projectRoot: cwd, kinds: ["user.suggestion.action"], limit: 10 });
	...
	it("/sasu-review works without manual goal setting and includes explicit evidence refs", async () => {
		await withTempProject(async (cwd) => {
			...
			await store.upsertWorkingState("last_checks", {
				failing: [{ name: "bun test tests/memory", files: ["src/memory/brief.ts"], status: "failed" }],
				passing: [],
				updatedAt: "2026-03-05T09:00:00.000Z",
			});
	...
	it("/sasu-goal manual override blocks low-confidence auto intent replacement", async () => {
		await withTempProject(async (cwd) => {
			...
			await store.upsertWorkingState("intent_hypotheses", {
				hypotheses: [{ label: "auto low-confidence guess", confidence: 0.2, evidence: ["fallback"] }],
			...
			const prompt = String(request?.message?.content ?? "");
			expect(prompt).toContain("Memory-selected intent: Manual focus lock (0.95 via manual_override)");
