    ...
    const brief = buildMissionBrief({
      workingState: {
        ...
        last_checks: {
          failing: [
            {
              name: "bun test tests/memory",
              files: ["src/memory/brief.ts", "tests/memory/brief.test.ts"],
            },
        ...
        open_risks: [
          {
            ...
            impact: 0.8,
            confidence: 0.9,
            evidence: ["src/memory/brief.ts"],
            nextStep: "Add reducer assertions",
          },
          {
            type: "coverage_gap",
            impact: 0.7,
            confidence: 0.8,
            evidence: ["tests/memory"],
          ...
          },
          {
            ...
            impact: 0.6,
            confidence: 0.7,
            evidence: ["src/review.ts"],
            nextStep: "Verify prompt contract",
          },
          {
            type: "perf",
            impact: 0.5,
            confidence: 0.6,
            evidence: ["src/memory/store.ts"],
            nextStep: "Profile sqlite calls",
          },
        ],
      },
    });
