...
function parseSuggestionLine(
  rawLine: string,
): Nullable<{ action: SuggestionAction; path: string; reason?: string }> {
  const listStripped = rawLine
    .trim()
    .replace(/^(?:[-*+]\s+|\d+[.)]\s+)/, "")
    .trim();
...
export function extractSuggestedFilesFromResponse(
  ...
  const normalizedCandidates = Array.from(
    new Set(
      candidatePaths
        .map((candidatePath) => normalizeSuggestedPath(candidatePath))
...
async function filterExistingProjectFiles(cwd: string, filePaths: string[]): Promise<string[]> {
  const unique = Array.from(
    new Set(filePaths.map((filePath) => filePath.trim().replace(/^\.\//, "")).filter(Boolean)),
  );
