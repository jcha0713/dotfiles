...
export async function loadSession(cwd: string): Promise<SessionData> {
	...
	let projectGoalSource = typeof parsed.projectGoalSource === "string" ? parsed.projectGoalSource : undefined;
	let sessionGoal = typeof parsed.sessionGoal === "string" ? parsed.sessionGoal : undefined;
	let sessionGoalSource = typeof parsed.sessionGoalSource === "string" ? parsed.sessionGoalSource : undefined;
...
export async function loadConfig(cwd: string): Promise<ConfigData> {
	...
	const checks = Array.isArray(parsed.checks)
		...
		: undefined;
	const openMode = parsed.openMode === "background" ? "background" : parsed.openMode === "foreground" ? "foreground" : undefined;
	const openCommand = typeof parsed.openCommand === "string" ? parsed.openCommand : undefined;
	const maxSuggestions = typeof parsed.maxSuggestions === "number" && parsed.maxSuggestions > 0 ? parsed.maxSuggestions : undefined;
