...
import type { FeedbackRecord, MemoryEvent, MemoryEventQuery, WorkingStateByKey, WorkingStateKey } from "./types";
...
`;
	...
	async appendEvents(events: MemoryEvent[]): Promise<void> {
		...
		const values = events
			.map(
				(event) => `(${toSqlLiteral(event.id)}, ${toSqlLiteral(event.ts)}, ${toSqlLiteral(event.projectRoot)}, ${toSqlLiteral(event.sessionId)}, ${toSqlLiteral(event.source)}, ${toSqlLiteral(event.kind)}, ${toJsonSqlLiteral(event.payload)}, ${toSqlLiteral(event.fingerprint)})`,
			)
			.join(",\n");
		const sql = `
BEGIN TRANSACTION;
...
`;
	...
	async getEventTotalCount(filters: { projectRoot?: string; sessionId?: string } = {}): Promise<number> {
		await this.ensureInit();
		const where: string[] = [];
		if (filters.projectRoot) where.push(`project_root = ${toSqlLiteral(filters.projectRoot)}`);
		if (filters.sessionId) where.push(`session_id = ${toSqlLiteral(filters.sessionId)}`);
...
`;
	...
	async getEventCountsByKind(filters: { projectRoot?: string; sessionId?: string } = {}): Promise<Record<string, number>> {
		await this.ensureInit();
		const where: string[] = [];
		if (filters.projectRoot) where.push(`project_root = ${toSqlLiteral(filters.projectRoot)}`);
		if (filters.sessionId) where.push(`session_id = ${toSqlLiteral(filters.sessionId)}`);
...
`;
	...
	async upsertWorkingState<K extends WorkingStateKey>(key: K, value: WorkingStateByKey[K]): Promise<void> {
		await this.ensureInit();
