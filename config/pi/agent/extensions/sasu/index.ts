...
import { buildNvimBufWriteEnvelope, normalizeNvimFeederEnvelope } from "./src/memory/feeder";
...
const NON_ACTIONABLE_SUGGESTION_PREFIXES = [
  ".sasu/",
  ".git/",
  "node_modules/",
  "dist/",
...
async function ensureSuggestedFileExists(
  cwd: string,
  filePath: string,
...
function rankOpenFileCandidates(
  ...
  const ranked = candidates
    ...
    .filter(
      (entry): entry is { candidate: OpenFileCandidate; score: number } =>
        typeof entry.score === "number",
    )
    .sort((a, b) => {
      if (b.score !== a.score) return b.score - a.score;
      if (a.candidate.action !== b.candidate.action)
        return a.candidate.action === "create" ? -1 : 1;
...
async function pickProjectFileWithFuzzyFilter(input: {
  ...
  if (!input.ctx.ui?.custom) {
    ...
    const options = ranked.slice(0, 30).map((candidate, index) => {
      const prefix =
        candidate.action === "create" ? "[CREATE] " : candidate.suggested ? "[SUGGESTED] " : "";
  ...
  const candidateByPath = new Map(
    input.candidates.map((candidate) => [candidate.path, candidate] as const),
  );

  const selected = await input.ctx.ui.custom<string | null>(
    (tui: any, theme: any, _kb: any, done: any) => {
      ...
      const updateFilterLine = () => {
        ...
        filterLine.setText(
          theme.fg(
            "dim",
            ` filter: ${filter || "(empty)"} • matches: ${count}/${input.candidates.length}`,
          ),
        );
      };
      ...
      const buildList = () => {
        ...
        const nextList = new SelectList(
          items,
          Math.min(items.length || 1, OPEN_PICKER_VISIBLE_ROWS),
          selectTheme,
        );
...
function buildSuggestionsChatBlock(
  suggestions: SuggestedFile[],
  limit = CHAT_SUGGESTIONS_LIMIT,
): string {
...
async function showSuggestionsAndOfferOpen(input: {
  ...
  const options = topSuggestions.map(
    (suggestion, i) => `${i + 1}. ${formatSuggestionLabel(suggestion)}`,
  );
  const skipOption = "Skip for now";
  const selected = await input.ctx.ui.select("SASU: open one suggested file now?", [
    ...options,
    skipOption,
  ]);
...
function buildMemoryStatusChatBlock(input: {
  ...
  const stateLines =
    input.workingStateKeys.length > 0
      ? input.workingStateKeys.map((key) => `- ${key}`)
      : ["- (none)"];
  return [
    "## SASU memory status",
    `DB: ${input.dbPath}`,
...
function buildMemoryTailChatBlock(
  events: Array<{ id: string; ts: string; kind: string; source: string; payload: unknown }>,
): string {
  if (events.length === 0) {
...
function parseNvimFeederEnvelopeArg(args: string): unknown {
  const trimmed = args.trim();
  if (!trimmed) {
    throw new Error(
      'Missing JSON event envelope. Usage: /sasu-memory-ingest-nvim {"source":"nvim","kind":"code.files.changed","payload":{...},"projectRoot":"...","ts":"..."}',
    );
  }
  try {
...
function asOptionalArgString(value: unknown): string | undefined {
  if (typeof value !== "string") return undefined;
  const trimmed = value.trim();
  return trimmed.length > 0 ? trimmed : undefined;
}

function parseNvimSaveSignalArgs(args: string): NvimSaveSignalArgs {
  const trimmed = args.trim();
  if (!trimmed) {
    throw new Error(
      'Missing file path. Usage: /sasu-memory-ingest-nvim-save <path> OR /sasu-memory-ingest-nvim-save {"file":"src/file.ts"}',
    );
  }
  ...
  if (trimmed.startsWith("{")) {
    ...
    try {
      ...
      throw new Error("Invalid JSON payload for nvim save signal");
    }

    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      throw new Error("Save signal payload must be a JSON object");
    }
    const record = parsed as Record<string, unknown>;
    const fileFromArray =
      Array.isArray(record.files) && typeof record.files[0] === "string"
        ? record.files[0]
        : undefined;
    const filePath =
      asOptionalArgString(record.file) ??
      asOptionalArgString(record.path) ??
      asOptionalArgString(fileFromArray);
    if (!filePath) {
      throw new Error("Save signal payload requires file/path (or files[0])");
    }

    return {
      filePath,
      ts: asOptionalArgString(record.ts),
      origin: asOptionalArgString(record.origin),
      reason: asOptionalArgString(record.reason),
      sessionId: asOptionalArgString(record.sessionId),
    };
  }

  const filePath = stripWrappingQuotes(trimmed).trim();
  if (!filePath) {
    throw new Error("Save signal path cannot be empty");
  }
  return { filePath };
}

function buildMemoryIngestNvimChatBlock(event: {
  id: string;
  ts: string;
  source: string;
  kind: string;
  ...
  const payloadLines = JSON.stringify(event.payload, null, 2).split(/\r?\n/);
  const payloadPreview = previewText(compactLines(payloadLines, 6).join(" "), 220);
  return [
...
export default function sasu(pi: ExtensionAPI) {
  ...
  const memoryStoreByCwd = new Map<
    string,
    Promise<Awaited<ReturnType<typeof createMemoryStore>>>
  >();
  ...
  const emitGitContextEvents = async (
    ctx: any,
    gitContext: GitContext,
    origin: string,
  ): Promise<void> => {
    const changedAreas = Array.from(
      new Set([...gitContext.changedFiles, ...gitContext.untrackedFiles]),
    );
  ...
  const resolveReviewMissionContext = async (input: {
    ...
    let resolvedIntentSource =
      fallbackIntent.length > 0 ? "fallback.intent" : input.fallbackFocusSource;
  ...
  const requestAgentSuggestions = async (input: {
    ...
    const candidatePaths = (await listProjectFiles(pi, input.cwd)).filter(
      isActionableSuggestionPath,
    );
    ...
    if (input.ctx.isIdle()) {
      ...
      if (!hasReadTool) {
        input.ctx.ui.notify(
          "SASU suggestion guard enabled with no read tool available (all tools disabled)",
          "warning",
        );
      }
  ...
  pi.registerCommand("sasu-review", {
    description:
      "Send current changes to Pi for a human-first review (use --show-prompt to debug prompt text)",
    handler: async (args, ctx) => {
      if (isBusyWaiting()) {
        ctx.ui.notify(
          "SASU is already waiting for a response. Please wait for it to finish.",
          "warning",
        );
        return;
      }
      ...
      const userIntent =
        explicitIntent ||
        missionContext.resolvedIntentLabel ||
        "No additional intent message provided.";
      ...
      const intentNeedsClarification =
        explicitIntent.length > 0 ? false : missionContext.needsClarification;
  ...
  pi.registerCommand("sasu-memory-ingest-nvim", {
    description: "Ingest a Neovim feeder event envelope JSON for phase-1 transport testing",
    handler: async (args, ctx) => {
      try {
        const rawEnvelope = parseNvimFeederEnvelopeArg(args);
        const normalized = normalizeNvimFeederEnvelope(rawEnvelope, ctx.cwd);
        const store = await getMemoryStore(ctx.cwd);
        const event = await ingestEvent(store, {
          ...normalized,
          sessionId: normalized.sessionId ?? getSessionId(ctx),
        });

        pi.sendMessage(
          {
            customType: "sasu-memory-ingest-nvim",
            content: buildMemoryIngestNvimChatBlock(event),
            display: true,
          },
          { triggerTurn: false },
        );
        ctx.ui.notify(`SASU memory ingested nvim event: ${event.kind}`, "info");
      } catch (error: any) {
        ctx.ui.notify(
          `Failed to ingest nvim feeder event: ${error?.message || "unknown error"}`,
          "error",
        );
      }
    },
  ...
  pi.registerCommand("sasu-memory-ingest-nvim-save", {
    description:
      "Ingest Neovim BufWritePost save signal (usage: /sasu-memory-ingest-nvim-save <path>)",
    handler: async (args, ctx) => {
      try {
        const parsed = parseNvimSaveSignalArgs(args);
        const normalized = buildNvimBufWriteEnvelope({
          filePath: parsed.filePath,
          projectRoot: ctx.cwd,
          ts: parsed.ts,
          origin: parsed.origin,
          reason: parsed.reason,
          sessionId: parsed.sessionId ?? getSessionId(ctx),
        });
        const store = await getMemoryStore(ctx.cwd);
        const event = await ingestEvent(store, normalized);

        pi.sendMessage(
          {
            customType: "sasu-memory-ingest-nvim-save",
            content: buildMemoryIngestNvimChatBlock(event),
            display: true,
          },
          { triggerTurn: false },
        );
        ctx.ui.notify(`SASU memory ingested nvim save event: ${parsed.filePath}`, "info");
      } catch (error: any) {
        ctx.ui.notify(
          `Failed to ingest nvim save signal: ${error?.message || "unknown error"}`,
          "error",
        );
      }
    },
  ...
  pi.registerCommand("sasu-memory-reset", {
    ...
    handler: async (args, ctx) => {
      ...
      if (!force) {
        ...
        const selected = await ctx.ui.select("Reset SASU memory for this project?", [
          "Reset",
          "Cancel",
        ]);
        if (selected !== "Reset") {
      ...
      if (mode === MODE_SESSION || mode === MODE_BOTH) {
        const enteredSessionFocus = (
          await ctx.ui.input(
            ...
            next.sessionGoal?.trim() ||
              next.projectGoal?.trim() ||
  ...
  pi.registerCommand("sasu-open", {
    ...
    handler: async (args, ctx) => {
      ...
      const candidates = Array.from(candidateByPath.values()).sort((a, b) =>
      ...
      if (selectedCandidate?.action === "create") {
        const createOption = "Create and open";
        const cancelOption = "Cancel";
        const confirmed = await ctx.ui.select(`SASU: create and open ${selectedPath}?`, [
          createOption,
          cancelOption,
        ]);
        if (confirmed !== createOption) {
  ...
  pi.on("agent_end", async (event: any, ctx) => {
    ...
    if (awaitingSuggestionResponse) {
      ...
      try {
        ...
        let suggestions = extractSuggestedFilesFromResponse(
          assistantText,
          candidatePaths,
          maxSuggestions,
        );
        ...
        await saveSession(ctx.cwd, {
          ...
          lastSuggestionsUpdatedAt:
            suggestions.length > 0 ? new Date().toISOString() : session.lastSuggestionsUpdatedAt,
        });

        if (suggestions.length === 0) {
          ctx.ui.notify(
            "Agent returned no valid OPEN/CREATE suggestions. Try /sasu-suggest again.",
            "warning",
          );
          return;
        }
    ...
    if (suggestions.length === 0) {
      ...
      ctx.ui.notify(
        "No cached suggestions. Run /sasu-suggest for agent-based file suggestions.",
        "info",
      );
      return;
    }
