...
type NvimResult = NvimContext | DiagnosticsResult | CurrentFunctionResult | SplitsResult | null;
 ...
 */
function severityColor(severity: DiagnosticItem["severity"]): "error" | "warning" | "dim" {
  switch (severity) {
    case "error":
      return "error";
    case "warning":
...
const NvimContextParams = Type.Object({
  action: StringEnum(["context", "diagnostics", "current_function", "splits"] as const, {
...
export function registerNvimContextTool(pi: ExtensionAPI, state: NvimConnectionState) {
  pi.registerTool({
    name: "nvim_context",
...
Use this tool when you need to know what the user is currently looking at in their editor.`,
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      try {
        ...
        } else if (errorMsg.includes("ECONNREFUSED") || errorMsg.includes("ENOENT")) {
      ...
      if (details.error) {
        ...
        switch (action) {
          case "context": {
            ...
            content = theme.fg("accent", filePath) + theme.fg("dim", `:${line}:${col}`);
            if (nvimCtx.filetype) {
          ...
          case "diagnostics": {
            ...
            const warnings = diags.filter((diag) => diag.severity === "warning").length;
            ...
            if (warnings > 0) {
              parts.push(theme.fg("warning", `${warnings} warning${warnings > 1 ? "s" : ""}`));
            }
            if (others > 0) {
            ...
            if (options.expanded) {
              for (const diag of diags) {
                ...
                content += theme.fg(severityColor(diag.severity), `[${diag.severity}]`);
          ...
          case "current_function": {
            ...
            content = theme.fg("accent", fn.name) + theme.fg("muted", ` (${fn.type})`);
            if (options.expanded) {
          ...
          case "splits": {
            ...
            const focusedCount = splits.filter((split) => split.is_focused).length;
            content = theme.fg("accent", `${splits.length} split${splits.length > 1 ? "s" : ""}`);
            if (focusedCount > 0) {
            ...
            if (options.expanded) {
              for (const split of splits) {
                ...
                const modified = split.modified ? theme.fg("warning", " [+]") : "";
                ...
                if (split.is_focused && split.cursor) {
                  content += theme.fg("dim", ` cursor ${split.cursor.line}:${split.cursor.col}`);
                }
              }
            }
            break;
